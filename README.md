...
The MCP (Model Context Protocol) Server empowers developers to write energy-efficient code, reducing the environmental and operational costs of software. By analyzing code and suggesting optimizations, it enables sustainable application development. Currently optimized for Java, with plans to support additional languages, it integrates seamlessly with tools like GitHub Copilot in Visual Studio Code.Using semantic search, the server performs static analysis on a dataset of energy efficiency observations, delivering research-backed suggestions (e.g., from academic and industry studies; see Acknowledgments) to enhance code generated by large language models (LLMs). Written in Python with FastMCP 2.x, it supports local operation via standard input/output (stdio) or remote access via Server-Sent Events (SSE). Data is stored in a JSON file by default or in a scalable, cloud-hosted Qdrant vector database for larger datasets. With configurable authentication for Development, Staging, and Production environments, and containerized deployment via Docker, the server is ready for secure and flexible use in diverse environments.
