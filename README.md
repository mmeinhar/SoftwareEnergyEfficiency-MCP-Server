...
The MCP (Model Context Protocol) Server helps developers write energy-efficient code, reducing the environmental and operational costs of software. By analyzing code and suggesting optimizations, it enables sustainable application development. Currently optimized for Java, with plans to support additional languages, it integrates with tools like GitHub Copilot in Visual Studio Code.

The server uses semantic search to analyze a dataset of energy efficiency observations, providing research-backed suggestions (e.g., from energy optimization studies; see Acknowledgments) that enhance code generated by large language models (LLMs). Built in Python with FastMCP 2.x, it supports local operation via standard input/output (stdio) or remote access via Server-Sent Events (SSE). Data is stored in a JSON file by default or in a scalable, cloud-hosted Qdrant vector database for larger datasets. With configurable authentication for Development, Staging, and Production environments and Docker-based deployment, the server ensures secure and flexible operation.
